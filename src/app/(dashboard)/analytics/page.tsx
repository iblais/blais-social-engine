...
export default function AnalyticsPage() {
  ...
  const load = useCallback(async () => {
    let query = supabase
      .from('posts')
      .select('id, caption, status, platform, published_at, scheduled_at, created_at')
      ...
      <Card>
        ...
        <CardContent>
          {sortedPosts.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              {/* Table header */}
              <div className="grid grid-cols-[1fr_100px_90px_180px_40px] gap-2 items-center px-3 py-2 text-xs font-medium text-muted-foreground border-b min-w-[500px]">
                <span>Caption</span>
                <button
              ...
              <div className="divide-y max-h-[500px] overflow-y-auto">
                {sortedPosts.map(p => (
                  <div
                    key={p.id}
                    className="grid grid-cols-[1fr_100px_90px_180px_40px] gap-2 items-center px-3 py-2.5 hover:bg-muted/50 transition-colors min-w-[500px]"
                  >
